...
function atualizarProgresso() {
  ...
  // Medidores gamificados (estimativa de economia ao otimizar a rota).
  const litros = (feitas * 0.9).toFixed(1);
  const co2 = (feitas * 2.3).toFixed(1);
  $("gauge-diesel").textContent = `${litros} L`;
  $("gauge-co2").textContent = `${co2} kg`;
  $("gauge-barra").style.width = pct.toFixed(1) + "%";
}
